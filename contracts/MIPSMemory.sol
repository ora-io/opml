...
contract MIPSMemory {
  ...
  // First read the size of data, the read the data
  function ReadMemoryToBytes(bytes32 stateHash, uint32 addr) public view returns (bytes memory) {
    require(addr & 3 == 0, "read memory must be 32-bit aligned");
    uint32 size = ReadMemory(stateHash, addr);
    require(size & 3 == 0, "data must be 32-bit aligned");
    bytes memory ret = new bytes(size);
    uint32 wordSize = size / 4;
    for (uint32 i = 0; i < wordSize; i += 4) {
      uint32 dat = ReadMemory(stateHash, addr+i);
      ret[i + 0] = bytes1(uint8(dat >> 24));
      ret[i + 1] = bytes1(uint8(dat >> 16));
      ret[i + 2] = bytes1(uint8(dat >> 8));
      ret[i + 3] = bytes1(uint8(dat >> 0));
    }
    return ret;
  }
